...
class _SigninState extends State<Signin> {
  ...
  void signinAction() {
    //url
    Uri url = Uri.parse("$BASE_URL/user/signin");

    //data to send
    Map<String, dynamic> userObject = {"username": username, "password": pwd};
    ...
    //request
    http
        .post(url, headers: headers, body: json.encode(userObject))
        .then((response) async {
      if (response.statusCode == 200) {
        final Map map = jsonDecode(response.body);
        final shared = await SharedPreferences.getInstance();
        shared.setString("Id", map['_id']);
